...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, embed_dim, heads=8, activation=None, forward_expansion=1, dropout=0.1):
        ...
        self.embed_dim = embed_dim
        self.norm1 = BatchChannelNorm(embed_dim)
        self.attention = Attention(embed_dim, heads, activation, dropout)
        self.norm2 =BatchChannelNorm(embed_dim)
...
class ResidualBlockSmall(nn.Module):
    def __init__(self, input_channels, out_channels, residual_downsample=None, stride=1):
        ...
        self.bnorm1 = BatchChannelNorm(out_channels)
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.bnorm2 = BatchChannelNorm(out_channels)
        self.activation = nn.ReLU()
...
class ResNetSmall(nn.Module):
    def __init__(self, layers, input_channels, classes):
        ...
        self.bnorm1 = BatchChannelNorm(64)
        self.activation = nn.ReLU()
    ...
    def _layer(self, num_layers, input_channels, output_channels, stride):
        ...
        if stride != 1:
            residual_downsample = nn.Sequential(
                ...
                BatchChannelNorm(output_channels * 4)
            )
...
class ResidualBlockLarge(nn.Module):
    def __init__(self, input_channels, out_channels, residual_downsample=None, stride=1, expansion=4):
        ...
        self.bnorm3 = BatchChannelNorm(out_channels * expansion)
        self.activation = nn.ReLU()
        self.residual_downsample = residual_downsample
...
def CIFAR100DataLoader(split, batch_size=8, num_workers=0, shuffle=True, size='32', normalize='standard'):
...
patch_size = 4
image_sz = 32
max_len = 10  # All sequences must be less than 1000 including class token
embed_dim = 512
classes = 100
layers = 12
channels = 3
...
for epoch in range(epochs):
    ...
    if (epoch+1)%5 == 0:
        torch.save({
            ...
            'train_acc': train_accs,
            'test_acc': test_accs
        }, './data' + model.name + '_CIFAR100_checkpoint.pt') 
